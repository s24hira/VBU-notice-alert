...
- **Intelligent Pre-Download Deduplication:** Checks Supabase before downloading notice files or triggering Gemini API calls, preventing redundant network requests and saving LLM quota.
- **Multi-Strategy Anti-Bot Bypass:** Uses a layered HTTP client (`cloudscraper` → `curl_cffi` → session warmup → fast-failing proxy list) with rotated User-Agents, full browser-grade headers, TLS fingerprint impersonation, and fast-fail fallback timeouts to reliably bypass WAF protections on the Samarth eGov portal without blocking worker threads.
